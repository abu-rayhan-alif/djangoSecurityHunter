# djsecinspect Architecture

This document explains the current architecture of `djsecinspect` and the intended evolution path for V1.
...
1. **CLI Layer** (`src/djsecinspect/cli.py`)
...
2. **Configuration Layer** (`src/djsecinspect/config.py`)
   - Reads `pyproject.toml` (`[tool.djsecinspect]`)
   - Reads `djsecinspect.toml` project override
   - Produces a normalized runtime config object

3. **Engine Layer** (`src/djsecinspect/engine.py`)
...
1. User runs `djsecinspect profile ...`
...
`djsecinspect` uses severity threshold gating:
...
- install package
- run tests
- generate SARIF (`djsecinspect scan --format sarif`)
...
1. Python entry points (`djsecinspect.rules`)
